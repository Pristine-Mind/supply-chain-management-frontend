      ...
      <section className="relative pt-12 pb-8 overflow-hidden">
        <div className="container mx-auto px-4 text-center">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-500/10 border border-primary-500/20 text-primary-500 text-[9px] font-black uppercase tracking-[0.3em] mb-4"
          >
            <Zap size={12} fill="currentColor" />
            Limited velocity
          </motion.div>

          <h2 className="text-3xl md:text-4xl font-black tracking-tighter italic uppercase italic text-secondary-900">
...
const FlashCard = ({ item, idx, onAdd, onNavigate }: any) => {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-primary-200 border border-white/5 rounded-2xl p-3 transition-all hover:border-primary-500/30"
    >
      {/* Mini Badge */}
      <div className="absolute top-4 left-4 z-20">
        <Badge variant="primary" size="sm" className="text-[8px] font-black uppercase tracking-wider gap-1">
      ...
      <div className="space-y-3">
        <div>
          ...
          <h3 className="text-xs text-primary-400 font-bold line-clamp-1 group-hover:text-primary-700 transition-colors">
            {name}
          </h3>
        </div>
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-[8px] font-bold uppercase text-secondary-500">
            <span>Stock</span>
            <span className="text-primary-500">{stockPercent}% left</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            {item.discounted_price && (
              <span className="text-[9px] text-primary-500 line-through">Rs. {item.listed_price}</span>
            )}
            <span className="text-sm font-black text-primary-500">Rs. {price.toLocaleString()}</span>
          </div>

          <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={(e) => { e.stopPropagation(); onAdd(); }}
            className="bg-white text-black p-2 rounded-xl hover:bg-primary-500 hover:text-white transition-all shadow-md"
          >
            <ShoppingBag size={14} />
          </motion.button>
        </div>
