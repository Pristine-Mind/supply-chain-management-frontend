    ...
    <div className="flex w-full p-6 gap-6">
      <div className="flex-none w-1/4 bg-gradient-to-b from-green-100 to-green-50 rounded-lg shadow-md p-6 flex items-center justify-center">
        <div className="text-center">
          <h3 className="text-2xl font-semibold text-accent-success-800 mb-2">Sell with Confidence</h3>
          <p className="text-base text-neutral-600">Reach thousands of potential buyers</p>
        </div>
      </div>

      <div className="flex-grow w-auto bg-white rounded-lg shadow border border-primary-500 p-10 mx-0">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            {errors.name && <p className="text-accent-error-500 mt-1">{errors.name.message}</p>}
          </div>
          ...
          <div>
            ...
            {errors.description && <p className="text-accent-error-500 mt-1">{errors.description.message}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              {errors.price && <p className="text-accent-error-500 mt-1">{errors.price.message}</p>}
            </div>
            <div>
              ...
              {errors.stock && <p className="text-accent-error-500 mt-1">{errors.stock.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div>
              ...
              <select {...register("category")} className="w-full bg-neutral-100 border border-neutral-300 p-3 rounded-lg text-base h-12">
              ...
              {errors.category && <p className="text-accent-error-500 mt-1">{errors.category.message}</p>}
            </div>
            <div>
              <label className="block mb-2 text-lg font-medium">Unit</label>
              <select {...register("unit")} className="w-full bg-neutral-100 border border-neutral-300 p-3 rounded-lg text-base h-12">
                <option value="">Select a unit</option>
                <option value="KG">Kilogram</option>
                <option value="LT">Liter</option>
              </select>
              {errors.unit && <p className="text-accent-error-500 mt-1">{errors.unit.message}</p>}
            </div>
            <div>
              <label className="block mb-2 text-lg font-medium">Location</label>
              <select {...register("location")} className="w-full bg-neutral-100 border border-neutral-300 p-3 rounded-lg text-base h-12">
                <option value="">Select location</option>
                {cities.map((city) => (
                  <option key={city.id} value={city.id}>{city.name}</option>
                ))}
              </select>
              {errors.location && <p className="text-accent-error-500 mt-1">{errors.location.message}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            {fields.map((fld, idx) => (
              <div key={fld.id} className="border p-4 mb-4 rounded-xl space-y-3">
                ...
                />
                {errors.images?.[idx]?.file && <p className="text-accent-error-500">{errors.images[idx].file!.message}</p>}
                ...
                {errors.images?.[idx]?.alt_text && <p className="text-accent-error-500">{errors.images[idx].alt_text!.message}</p>}
                ...
                {errors.images?.[idx]?.order && <p className="text-accent-error-500">{errors.images[idx].order!.message}</p>}
                <button type="button" onClick={() => remove(idx)} className="bg-accent-error-600 text-white px-4 py-2 rounded-lg">
                  Remove Image
                </button>
              </div>
            ))}
            <button type="button" onClick={() => append({ file: null, alt_text: "", order: fields.length })} className="bg-secondary-600 text-white px-4 py-2 rounded-lg">
              + Add Another Image
            </button>
          </div>

          <button type="submit" className="w-full bg-primary-600 text-white py-4 rounded-xl text-lg">
            Submit Product
          </button>

          {submitError && <p className="text-accent-error-600 mt-2">Error: {submitError}</p>}
        </form>
      </div>
      ...
      <div className="flex-none w-1/4 bg-gradient-to-b from-orange-50 to-orange-100 rounded-lg shadow-md p-6 flex items-center justify-center">
        <div className="text-center">
          <h3 className="text-2xl font-semibold text-primary-800 mb-2">Tips for Sellers</h3>
          <ul className="text-base text-neutral-600 space-y-2 text-left">
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-xl shadow-lg text-center">
            <h4 className="text-accent-success-600 font-bold mb-4">Product added successfully!</h4>
            <button onClick={() => { setShowSuccess(false); navigate("/home"); }} className="bg-secondary-600 text-white px-6 py-3 rounded-lg text-lg">
              OK
            </button>
          </div>
        </div>
