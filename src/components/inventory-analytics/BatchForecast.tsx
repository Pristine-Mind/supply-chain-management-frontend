  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-600 to-teal-600 shadow-lg rounded-lg p-6 text-white">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-accent-success-100 mt-2">
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <h2 className="text-xl font-bold text-neutral-800 mb-4">Configure Forecast</h2>
      ...
      {errorMessages.length > 0 && (
        <div className="bg-accent-warning-50 border border-accent-warning-200 rounded-lg p-4">
          <h3 className="font-semibold text-accent-warning-800 mb-2">⚠️ Warnings</h3>
          <ul className="space-y-1">
            {errorMessages.map((error, idx) => (
              <li key={idx} className="text-sm text-accent-warning-700">
                {error}
              </li>
            ))}
          </ul>
      ...
      {errors.batchForecast && (
        <div className="bg-accent-error-50 border border-accent-error-200 rounded-lg p-4 text-accent-error-700">
          {errors.batchForecast}
        </div>
      )}
        ...
        <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-200">
                  ...
                  <th className="text-center py-3 px-4 text-sm font-medium text-neutral-500">Daily Forecast</th>
                  <th className="text-center py-3 px-4 text-sm font-medium text-neutral-500">{days}-Day Total</th>
                  <th className="text-center py-3 px-4 text-sm font-medium text-neutral-500">Method</th>
                  <th className="text-right py-3 px-4 text-sm font-medium text-neutral-500">Action</th>
                </tr>
              </thead>
              <tbody>
                {forecasts.map((item) => (
                  <tr key={item.product_id} className="border-b border-neutral-100 hover:bg-neutral-50">
                    <td className="py-3 px-4">
                      <span className="font-medium text-neutral-900">{item.product_name}</span>
                      <p className="text-xs text-neutral-500">ID: {item.product_id}</p>
                    </td>
                    ...
                    <td className="text-center py-3 px-4">
                      <span className="font-semibold text-accent-success-600">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-secondary-50 rounded-lg p-4 text-center">
              <p className="text-sm text-neutral-600">Total Daily Demand</p>
              <p className="text-2xl font-bold text-secondary-600">
                {forecasts.reduce((sum, f) => sum + f.forecast.daily_forecast, 0).toFixed(1)}
              </p>
              <p className="text-xs text-neutral-500">units/day</p>
            </div>
            <div className="bg-accent-success-50 rounded-lg p-4 text-center">
              <p className="text-sm text-neutral-600">Total {days}-Day Demand</p>
              <p className="text-2xl font-bold text-accent-success-600">
                {forecasts.reduce((sum, f) => sum + f.forecast.total_forecast, 0)}
              </p>
              <p className="text-xs text-neutral-500">units</p>
            </div>
            <div className="bg-secondary-50 rounded-lg p-4 text-center">
              <p className="text-sm text-neutral-600">Products Forecasted</p>
              <p className="text-2xl font-bold text-secondary-600">{forecasts.length}</p>
