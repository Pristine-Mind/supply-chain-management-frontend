    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <p className="text-neutral-600">Loading Inventory Analytics Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  if (errors.portfolioAnalytics || !portfolioAnalytics) {
    return (
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="p-4 bg-accent-error-50 border border-accent-error-200 rounded-md text-accent-error-700">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white shadow-md rounded-lg p-6 border-l-4 border-secondary-500">
          ...
          <div className="text-4xl font-bold text-neutral-900">{portfolio_analytics.total_products}</div>
          <Link to="/inventory-analytics/products" className="text-sm text-secondary-600 hover:underline mt-2 inline-block">
            View All →
          </Link>
        </div>

        <div className="bg-white shadow-md rounded-lg p-6 border-l-4 border-accent-success-500">
          <h3 className="text-sm font-medium text-neutral-500 mb-2">Healthy Stock</h3>
          <div className="text-4xl font-bold text-accent-success-600">
        ...
        <div className="bg-white shadow-md rounded-lg p-6 border-l-4 border-accent-warning-500">
          <h3 className="text-sm font-medium text-neutral-500 mb-2">Low Stock</h3>
          <div className="text-4xl font-bold text-accent-warning-600">{portfolio_analytics.low_stock_count}</div>
          <p className="text-sm text-neutral-500 mt-2">Products need attention</p>
        </div>

        <div className="bg-white shadow-md rounded-lg p-6 border-l-4 border-accent-error-500">
          <h3 className="text-sm font-medium text-neutral-500 mb-2">Critical Risk</h3>
          <div className="text-4xl font-bold text-accent-error-600">{portfolio_analytics.stockout_risk_count}</div>
          <Link to="/inventory-analytics/reorder-recommendations?risk_level=critical" className="text-sm text-secondary-600 hover:underline mt-2 inline-block">
            View Urgent →
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Risk Distribution */}
        <div className="bg-white shadow-md rounded-lg p-6">
          <h3 className="text-xl font-bold text-neutral-800 mb-4">Portfolio Risk Distribution</h3>
        ...
        {/* At-Risk Products */}
        <div className="bg-white shadow-md rounded-lg p-6">
          <h3 className="text-xl font-bold text-neutral-800 mb-4">Days Until Stockout (Top 5)</h3>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-bold text-neutral-800">At-Risk Products</h3>
          <Link
            to="/inventory-analytics/reorder-recommendations"
            className="px-4 py-2 bg-secondary-600 text-white rounded-lg hover:bg-secondary-700 transition text-sm"
          >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-200">
                  ...
                  <th className="text-right py-3 px-4 text-sm font-medium text-neutral-500">Action</th>
                </tr>
              </thead>
              <tbody>
      ...
      {recommendations.length > 0 && (
        <div className="bg-white shadow-md rounded-lg p-6">
          <h3 className="text-xl font-bold text-neutral-800 mb-4">Priority Reorder Recommendations</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {recommendations.slice(0, 6).map((rec) => (
              <div
                key={rec.product_id}
                className={`p-4 rounded-lg border-2 ${
                  rec.risk_level === 'critical' ? 'border-accent-error-200 bg-accent-error-50' :
                  rec.risk_level === 'high' ? 'border-primary-200 bg-primary-50' :
                  'border-accent-warning-200 bg-accent-warning-50'
                }`}
              >
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-semibold text-neutral-900 truncate" title={rec.product_name}>
                    {rec.product_name}
                  </h4>
                ...
                <p className="text-sm text-neutral-600 mb-1">SKU: {rec.sku}</p>
                <div className="flex justify-between items-center text-sm mb-2">
                  <span className="text-neutral-600">Stock: <strong>{rec.current_stock}</strong></span>
                  <span className="text-neutral-600">Days: <strong className={rec.days_until_stockout <= 3 ? 'text-accent-error-600' : ''}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Link
                    ...
                    className="text-xs px-3 py-1 bg-secondary-600 text-white rounded hover:bg-secondary-700 transition"
                  >
                    Details
                  </Link>
                </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link
            ...
            className="px-4 py-3 bg-secondary-600 text-white rounded-lg hover:bg-secondary-700 transition text-center font-medium"
          >
            📦 Products
          </Link>
          <Link
            to="/inventory-analytics/reorder-recommendations"
            className="px-4 py-3 bg-accent-error-600 text-white rounded-lg hover:bg-accent-error-700 transition text-center font-medium"
          >
            🚨 Reorder Now
          </Link>
          <Link
            to="/inventory-analytics/batch-forecast"
            className="px-4 py-3 bg-accent-success-600 text-white rounded-lg hover:bg-accent-success-700 transition text-center font-medium"
          >
            📈 Batch Forecast
          </Link>
          <Link
