...
import { ShoppingBag, ArrowUpRight, PackageX } from 'lucide-react';
...
const FeaturedProducts: React.FC<FeaturedProductsProps> = ({ 
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((item) => {
      const name = item.product_details?.name?.toLowerCase() || "";
      const category = item.product_details?.category_details?.toLowerCase() || "";
          ...
          <AnimatePresence mode='popLayout'>
            {filteredProducts.length > 0 ? (
              ...
              >
                ...
                <p className="text-sm">Try searching for a different keyword or category.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
