  ...
  return (
    <div className="min-h-screen bg-gray-100 w-full p-4">
      <div className="p-8 bg-white rounded-xl shadow-sm mb-8">
        <h1 className="text-2xl font-bold text-primary-700 mb-6">Purchase Orders</h1>
        {/* Loading indicator */}
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500 mr-4"></div>
            <span className="text-base text-gray-500">Loading purchase orders...</span>
          </div>
        ) : (
            ...
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-lg shadow-lg p-5 flex flex-col space-y-3 border border-gray-200 transition-transform transform hover:scale-105">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  ...
                  order.sent_to_vendor ? 'bg-yellow-100 text-yellow-700' :
                  'bg-red-100 text-red-700'
                }`}>
                  {order.approved ? 'Approved' : order.sent_to_vendor ? 'Pending' : 'Rejected'}
                </span>
              </div>
            ))}
          </div>
