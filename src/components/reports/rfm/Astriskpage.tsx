...
import {
  AlertTriangle, DollarSign, Users, TrendingDown,
...
const AtRiskPage: React.FC = () => {
  const customers = DUMMY_CUSTOMERS.filter(c => c.segment === 'At Risk');
  ...
  const critCount  = customers.filter(c => c.churnRisk === 'Critical').length;
  ...
  const Th: React.FC<{ col: SortKey; label: string }> = ({ col, label }) => (
    <th onClick={() => handleSort(col)}
      className="px-4 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:text-orange-600 select-none">
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🟡 At Risk</h1>
          <p className="text-sm text-gray-400">
            Declining activity · High churn probability — needs immediate retention action
          </p>
        </div>
        <span className="ml-auto bg-orange-100 text-orange-700 text-xs font-bold px-3 py-1 rounded-full">
          Discount Rescue campaign
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'At Risk Customers', value: customers.length.toString(),      sub: 'In segment',        icon: <Users size={16} className="text-orange-600" />,    bg: 'bg-orange-50'  },
          { label: 'Avg Churn Score',   value: `${avgChurn}%`,                   sub: 'High = danger',     icon: <AlertTriangle size={16} className="text-rose-600" />, bg: 'bg-rose-50'  },
          { label: 'Revenue at Stake',  value: `$${totalRev.toLocaleString()}`,  sub: 'Total segment rev', icon: <DollarSign size={16} className="text-amber-600" />,  bg: 'bg-amber-50'   },
          { label: 'Critical Tier',     value: critCount.toString(),             sub: 'Need urgent action',icon: <TrendingDown size={16} className="text-red-600" />,  bg: 'bg-red-50'     },
        ].map(k => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-wrap gap-3 items-center justify-between">
          <h3 className="font-bold text-gray-800">At Risk Customers</h3>
          <div className="flex flex-wrap gap-2 items-center">
            <div className="relative">
              <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" placeholder="Search…" value={search}
                onChange={e => { setSearch(e.target.value); setPage(1); }}
                className="pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-300 w-40" />
            </div>
          ...
          <table className="w-full min-w-[750px]">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Campaign</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {paginated.length === 0 ? (
                ...
                <tr key={c.id} className="hover:bg-orange-50/40 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 bg-gray-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${c.churnScore < 0.55 ? 'bg-amber-500' : c.churnScore < 0.70 ? 'bg-orange-500' : 'bg-rose-600'}`}
                  ...
                  <td className="px-4 py-3">
                    <span className="text-[10px] bg-orange-50 text-orange-700 border border-orange-200 px-2 py-0.5 rounded font-medium whitespace-nowrap">
                      {c.campaignType}
                    </span>
                  </td>
                </tr>
